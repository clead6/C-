...
/*
    ...
    {
        if (row_number==matrix.row_number && column_number==matrix.column_number) {
            matrix_2d addition_array(row_number,column_number);
            for (int i{0}; i<(matrix_size); i++) {
                addition_array[i]=array[i]+matrix.array[i];
            }
            return addition_array;
        }
        else {
            throw "Operation not possible, matrices must be the same size.";
        }
    }
    ...
    { 
        if (row_number==matrix.row_number && column_number==matrix.column_number) {
            matrix_2d substraction_array(row_number,column_number);
            for (int i{0}; i<(matrix_size); i++) {
                substraction_array[i]=matrix.array[i]-matrix[i];
            }
            return substraction_array;
        }
        else {
            throw "Operation not possible, matrices must be the same size.";
        }
